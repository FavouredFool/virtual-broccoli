...
public class DrawerWithItems : MonoBehaviour
{
    [Header("Interaction")]
    ...
    [SerializeField] private GameObject insideDrawer;

    private readonly List<GameObject> itemsOfDrawer;
    ...
    protected virtual void OnEnable()
    {
        ...
        for (int i = 0; i < insideDrawer.transform.childCount; i++)
        {
            Transform child = insideDrawer.transform.GetChild(i);

            XRGrabInteractable grabbableItem = child.GetComponent<XRGrabInteractable>();

            if (grabbableItem != null)
            {
                grabbableItem.enabled = false;
                child.GetComponent<Rigidbody>().isKinematic = true;
                itemsOfDrawer.Add(child.gameObject);
            }
        }
    }
    ...
    private void UpdateDrawer()
    {
        ...
        //If drawer is opened for a certain amount, items can be collected
        if (currentPercentage > 0.75)
        {
            foreach (GameObject itemInside in itemsOfDrawer)
            {
                itemInside.GetComponent<XRGrabInteractable>().enabled = true;
                itemInside.GetComponent<Rigidbody>().isKinematic = false;
            }
        }
    }
